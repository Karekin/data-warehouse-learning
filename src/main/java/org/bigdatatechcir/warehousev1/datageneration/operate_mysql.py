...
def run():
    ...
    while True:
        ...
        send_to_mysql(customer_login_sql, customer_login_params)

        # 写入 product_brand_info
        product_brand_info_tuple = generate_product_brand_info.return_product_brand_info('mysql')
        ...
        send_to_mysql(product_brand_info_sql, product_brand_info_tuple)

        # 写入 product_supplier_info
        product_supplier_info_tuple = generate_product_supplier_info.return_product_supplier_info('mysql')
        ...
        send_to_mysql(product_supplier_info_sql, product_supplier_info_tuple)

        # 写入 warehouse_shipping_info
        warehouse_shipping_info_tuple = generate_warehouse_shipping_info.return_warehouse_shipping_info('mysql')
        warehouse_shipping_info_sql = (
            "insert into warehouse_shipping_info(ship_name, ship_contact, telephone, price) values (%s, %s, %s, %s)")
        send_to_mysql(warehouse_shipping_info_sql, warehouse_shipping_info_tuple)
        ...
        # 写入customer_inf
        customer_inf_params = generate_customer_inf.return_customer_inf("mysql")
        ...
        customer_inf_sql = ("insert into mall.customer_inf ( customer_name, identity_card_type, identity_card_no, "
                            ...
                            "%s, %s)")
        send_to_mysql(customer_inf_sql, customer_inf_params_tuple)

        # 写入customer_addr
        customer_addr_params = generate_customer_addr.return_customer_addr('mysql')
        ...
        send_to_mysql(customer_addr_sql, customer_addr_params_tuple)

        # 写入 customer_login_log
        customer_login_log_params = generate_customer_login_log.return_customer_login_log("mysql")
        ...
        customer_login_log_sql = ("insert into mall.customer_login_log(login_time, login_ip, login_type, customer_id) "
                                  "values (%s, %s, %s, %s)")
        send_to_mysql(customer_login_log_sql, customer_login_log_params_tuple)

        # 写入 product_info
        product_info_params = generate_product_info.return_product_info('mysql')
        ...
        send_to_mysql(product_info_sql, product_info_params_tuple)
        ...
        # 写入order_master
        order_master_params = generate_order_master.return_order_master('mysql')
        ...
        # 写入order_cart
        order_cart_params = generate_order_cart.return_order_cart('mysql')
        ...
        order_cart_sql = ("insert into order_cart(product_amount, price, add_time, customer_id, product_id) values ("
                          "%s, %s, %s, %s, %s) ")
        send_to_mysql(order_cart_sql, order_cart_params_tuple)
